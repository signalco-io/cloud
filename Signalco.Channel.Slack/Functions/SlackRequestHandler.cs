...
internal class SlackRequestHandler : ISlackRequestHandler
{
    ...
    {
        secrets = secretsProvider ?? throw new ArgumentNullException(nameof(secretsProvider));
        this.logger = logger ?? throw new ArgumentNullException(nameof(logger));
    }

    public async Task VerifyFromSlack(HttpRequest req, CancellationToken cancellationToken)
    {
